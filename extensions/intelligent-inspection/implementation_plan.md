...
| Planned implementers | Phase 5A, Phase 5B, Phase 6, Phase 7, Phase 8, and Phase 9A AI & Algorithm Developer: Nouran Ismail; Phase 9B and later implementation assignments remain **TBD** |
...
**Amendment status:** **APPROVED — Nouran Ismail, Project Owner, 2026-09-10**. The primary demonstration remains UAV pipeline inspection. No motor, pump, or bearing project is introduced. Phase 9A is **AUTHORIZED — NOT STARTED** and assigned to Nouran Ismail — AI & Algorithm Developer. Phase 9B is **APPROVED IN PRINCIPLE BUT NOT AUTHORIZED FOR IMPLEMENTATION**. Phases 10–17 remain **NOT AUTHORIZED**.
...
- **Authorization:** **AUTHORIZED — NOT STARTED**. Implementation is limited to the six-file allowlist. Phase 9B cannot begin until Phase 9A developer verification passes and Project Owner review accepts it.
...
- **Objective:** Map governed UAV pipeline CV, thermal, gas, pressure, and inspection-history information into `NumericalFeatureSet`; generate a governed synthetic degradation dataset; train and compare regression models for a synthetic 30-day health score; demonstrate the unchanged Phase 9A interface.
...
- **Prerequisites:** Phase 9A accepted; this feature catalog, unit-code mapping, generator, target, horizon, dataset governance, split, leakage controls, candidates, metrics, thresholds, uncertainty method, products, and implementer explicitly approved.
...
- **Feature catalog:** IDs 101–115 exactly as proposed in `architecture/interface_contracts.md`; ascending deterministic order; invalid entries zero and invalid; no raw image and no inferred anomaly geometry.
- **Synthetic-generation assumptions:** Generator version 1 and seed `20260910`; 400 synthetic asset groups with eight observations at days `[0,30,60,90,120,150,180,210]` (3,200 rows). Per asset, initial latent burden `s0~Uniform(0,0.65)` and daily progression `r~Uniform(0,0.0015)` produce `s=clip(s0+r*day,0,1)`. Finding presence is a seeded Bernoulli draw with probability `0.05+0.90*s`; confidence when present is `clip(0.50+0.45*s+N(0,0.03),0,1)`. Temperature difference is `clip(80*s+N(0,5),-160,380)` degrees Celsius; gas concentration is `clip(100000*s^2+N(0,2500),0,1.0e6)` ppm; internal pressure is `clip(5000*(1-0.30*s)+N(0,100),0,1.0e5)` kPa; pressure-change rate is `clip(-500*s+N(0,25),-5.0e4,5.0e4)` kPa/hour. Inspection age follows the observation interval; prior finding count accumulates seeded findings; prior health is the prior generated target; maintenance age increases by observation interval and resets only under the recorded seeded maintenance rule `s>=0.85`. Location fields are generated inside their approved ranges but do not drive the target. Controlled missing and out-of-distribution records are generated in a separately labelled verification slice and excluded from fitting. These distributions are synthetic assumptions, not measured asset behavior.
- **Target:** Horizon is 30 days through context ID `9001`. Define normalized factors `f1=presence*confidence`, `f2=clip(max(temperatureDifference,0)/100,0,1)`, `f3=clip(gasConcentration/100000,0,1)`, `f4=clip(abs(internalPressure-5000)/5000,0,1)`, `f5=clip(abs(pressureChangeRate)/1000,0,1)`, `f6=clip(inspectionAge/3650,0,1)`, `f7=clip(priorFindingCount/20,0,1)`, `f8=clip((100-priorHealthScore)/100,0,1)`, and `f9=clip(timeSinceMaintenance/3650,0,1)`. Then `B=0.20*f1+0.12*f2+0.12*f3+0.12*f4+0.10*f5+0.06*f6+0.08*f7+0.12*f8+0.08*f9`, and `healthScore30Day=clip(100*(1-B)-30*(0.02+0.18*B)+epsilon,0,100)`, with seeded `epsilon=clip(N(0,1),-2,2)`. The equations, distributions, constants, weights, and seed shall be frozen and versioned before generation. `priorHealthScore` may be used only when genuinely available at inference time and shall never contain the future target. Every emitted valid health value is explicitly bounded to `[0,100]`.
- **Split/leakage control:** Allocate asset groups before generating or fitting any data-dependent transformation. Use deterministic group-safe 70% train / 15% validation / 15% test allocation by stable SHA-256 ordering of synthetic asset ID plus seed/configuration; stratify groups by terminal health-score bands `[0,25)`, `[25,50)`, `[50,75)`, and `[75,100]`. All eight rows for one asset remain in exactly one partition, yielding 280/60/60 assets and 2,240/480/480 rows. Within validation, stable ordering reserves a disjoint 40 assets/320 rows for candidate selection and 20 assets/160 rows for uncertainty calibration. Test shall not influence feature selection, model selection, hyperparameters, thresholds, preprocessing decisions, or conformal calibration and remains untouched until architecture, model parameters, preprocessing, and thresholds are frozen.
- **Models:** Median-target baseline; at most three candidates—regularized linear regression, regression tree, and bagged-tree ensemble—selected using validation MAE, then RMSE, then higher R-squared. No test-guided tuning.
- **Uncertainty:** Deterministic 90% split-conformal interval from absolute residuals on the reserved calibration subset; report coverage and mean width; invalid/OOD input abstains.
- **Locked proposed criteria:** Synthetic test MAE `<=8.0`; RMSE `<=12.0`; R-squared `>=0.65`; 90% interval coverage `[0.80,0.98]`; zero schema violations; zero uncontrolled failures. Report baseline and each candidate on train/validation and the frozen selection once on test.
...
- **Hold point:** **APPROVED IN PRINCIPLE BUT NOT AUTHORIZED FOR IMPLEMENTATION**. Phase 9B data generation, fitting, and evaluation require Phase 9A developer verification PASS, Project Owner acceptance, and separate execution authorization. The exact disclaimer shall appear in every Phase 9B dataset, model-card, and evidence artifact. Phases 10–17 remain unauthorized.
