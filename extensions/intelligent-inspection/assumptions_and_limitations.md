...
The authorized UAV Pipeline Configuration is limited to RGB camera output and simulation context metadata from the installed pregenerated MathWorks example “Simulate Simple Flight Scenario and Sensor in Unreal Engine Environment.” MATLAB R2026a, Simulink, UAV Toolbox, Simulink 3D Animation, and Computer Vision Toolbox are installed, licensed, and approved for Phase 13. The example shall not be modified. The demonstration is not evidence of real-world inspection, anomaly-detection, pipeline-condition, flight, or production readiness and does not validate camera realism, domain transfer, environmental robustness, anomaly taxonomy completeness, flight behavior, or MissionSupervisor compatibility. No direct UAV or MissionSupervisor integration is authorized.
