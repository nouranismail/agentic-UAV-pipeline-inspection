...
| IIW-AC-061 | The fixed regression implementation does not inspect the test partition before freeze. | A median-target baseline is reported only as a reference; one regularized linear-regression model with a frozen configuration is fitted using training data, checked using validation data, and evaluated on test only after generator, transformations, model, and uncertainty policy are frozen. No tree, ensemble, multi-model comparison, hyperparameter search, or test-guided selection occurs. |
...
| IIW-AC-063 | Residual-based uncertainty is deterministic and controlled. | The frozen model records a deterministic uncertainty score equal to `min(validationRMSE/100,1)` using validation residuals only; repeated calculations are identical, the test partition does not set or calibrate uncertainty, and invalid/OOD inputs abstain. |
...
`IIW-AC-054` through `IIW-AC-065` remain approved as amended for the simplified Phase 9B demonstration. Acceptance additionally requires frozen/versioned target equations and noise distributions before generation; group assignment before data-dependent transformation; zero group overlap; test isolation until the model is frozen; inference-time availability and no future-target leakage for `priorHealthScore`; exclusion of location coordinates from causal regression predictors; `[0,100]` prediction bounding; and controlled invalid/review status for unsupported or out-of-distribution input. The locked regression thresholds remain synthetic-test MAE `<=8.0`, RMSE `<=12.0`, and R-squared `>=0.65`, with zero interface violations and zero uncontrolled failures.
