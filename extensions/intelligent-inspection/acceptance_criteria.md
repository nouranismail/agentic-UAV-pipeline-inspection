...
**Status:** **APPROVED — PHASE 15 AUTHORIZED, NOT STARTED / CORRECTIVE PREFLIGHT AUTHORIZED.** Nouran Ismail — Project Owner / Lead Systems Engineer / MBD Architect, 2026-09-17. Phase 15 is read-only evaluation except for `reusability_evaluation.md`, `reusable_core_hashes.txt`, and `test_reusability.m`. Phases 13 and 14 are frozen accepted evidence.
...
Acceptance is locked to: two or more configurations with identical reusable-core paths and individual file hashes; zero project-semantic core changes; complete mandatory interface/direction equality; zero consumer-interface changes for detector substitution; exactly three Phase 15 artifacts changed; and zero uncontrolled failures. The canonical aggregate input is the concatenation, in ordinal bytewise ascending normalized-path order, of UTF-8-without-BOM `<repository-relative-forward-slash-path>\n<lowercase-64-hex-file-sha256>\n` entries, including the final LF and containing no CR, spaces, extra delimiters, or blank lines. Hash evidence must record both labeled aggregates, all normalized inventory paths and individual hashes, this exact serialization, SHA-256 algorithm, file count, branch, commit, and generation date. The stopped preflight and its ordering-convention cause must be retained in later Phase 15 evidence. Any additional artifact, dependency, dataset, model, core/configuration/interface modification, or implementation need is a stop condition. Phase 15 remains **AUTHORIZED — NOT STARTED / CORRECTIVE PREFLIGHT AUTHORIZED**. Phases 16–17 remain **NOT AUTHORIZED**.
